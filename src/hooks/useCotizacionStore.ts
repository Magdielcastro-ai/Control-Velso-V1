...
export const useCotizacionStore = () => {
  ...
  const guardarCotizacion = useCallback(async (estado: CotizacionGuardada['estado'] = 'borrador') => {
    ...
    const guardada: CotizacionGuardada = {
      ...
      clienteNombre: nuevaCotizacion.datosCliente.empresa || nuevaCotizacion.datosCliente.nombre || 'Sin cliente',
    ...
    if (user) {
      try {
        const cotizacionDB = {
          ...
          cliente_nombre: nuevaCotizacion.datosCliente.empresa || nuevaCotizacion.datosCliente.nombre || 'Sin cliente',
    ...
    try {
      ...
      if (data) {
        ...
        const cotizacionCargada: Cotizacion = {
          ...
          subtotalMXN: Number(data.subtotal_mxn) || (Number(data.subtotal) || 0),
          ivaPorcentaje: Math.round(Number(data.iva_porcentaje)) || 16,
          iva: Number(data.iva) || ((Number(data.subtotal) || 0) * (Math.round(Number(data.iva_porcentaje)) || 16) / 100),
          ivaMXN: Number(data.iva_mxn) || ((Number(data.subtotal) || 0) * (Math.round(Number(data.iva_porcentaje)) || 16) / 100),
          total: Number(data.total) || 0,
          totalMXN: Number(data.total_mxn) || (Number(data.total) || 0),
          margenUtilidad: Math.round(Number(data.margen_utilidad)) || 30,
        };
